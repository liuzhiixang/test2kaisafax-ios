...
//  kaisafax
//
//  Created by semny on 16/8/31.
//  Copyright © 2016年 kaisafax. All rights reserved.
//

#import "SQScanWrapper.h"
#import "SQScanNative.h"
#import "ZXingWrapper.h"
#import <Photos/Photos.h>

@interface SQScanWrapper()
{
    
}

//ios7之后native封装
@property(nonatomic,strong)SQScanNative* scanNativeObj;
...
- (instancetype)initWithPreView:(UIView*)preView ArrayObjectType:(NSArray*)arrayBarCodeType cropRect:(CGRect)cropRect
                        success:(void(^)(NSArray<SQScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
        ...
        if ( [SQScanWrapper isSysIos7Later] )
        {
            _scanNativeObj = [[SQScanNative alloc]initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray<SQScanResult*> *array) {
        ...
        else
        {
            _scanZXingObj = [[ZXingWrapper alloc]initWithPreView:preView block:^(ZXBarcodeFormat barcodeFormat, NSString *str, UIImage *scanImg) {
                ...
                if ( blockScanResult )
                {
                    NSString *barCodeType = [SQScanWrapper convertZXBarcodeFormat:barcodeFormat];
                    
                    SQScanResult *result = [[SQScanResult alloc]initWithScanString:str imgScan:scanImg barCodeType:barCodeType];
...
/**
 ...
 @return SQScanVendor
 */
- (instancetype)initZXingWithPreView:(UIView *)preView success:(void(^)(NSArray<SQScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
...
- (void)startScan
{
    ...
    if ( [SQScanWrapper isSysIos7Later] && !_isUseZXingLib )
 ...
 *  @param block 返回识别结果
 */
+ (void)recognizeImage:(UIImage*)image success:(void(^)(NSArray<SQScanResult*> *array))block;
{
    ...
    [ZXingWrapper recognizeImage:image block:^(ZXBarcodeFormat barCodeFormat,NSString* str)
     {
         ...
         if (block) {
             ...
             SQScanResult *result = [[SQScanResult alloc]initWithScanString:str imgScan:tmpImg barCodeType:barCodeType];
             block(@[result]);
         }
...
+ (UIImage*)createQRWithString:(NSString*)str size:(CGSize)size
{
    if ([SQScanWrapper isSysIos7Later])
    {
        return [SQScanWrapper createNonInterpolatedUIImageFormCIImage:[SQScanWrapper createQRForString:str] withSize:size.width];
    }
    else
    {
...
{
    ZXBarcodeFormat zxformat = [SQScanWrapper convertCodeFomratToZXBarcodeFormat:format];
