...
@interface SQZXCapture ()<CALayerDelegate>
...
- (void)setDelegate:(id<SQZXCaptureDelegate>)delegate {
  _delegate = delegate;
    ...
    if (!self.cameraIsReady) {
      ...
      if ([self.delegate respondsToSelector:@selector(captureCameraIsReady:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
          [self.delegate SQCaptureCameraIsReady:self];
        });
      }
    }
