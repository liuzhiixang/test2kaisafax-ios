...
- (NSString *)textWithEmoji {
    ...
    if (self.type == JXMessageTypeForeseeComposing) {    // 预知消息添加前缀
        ret = [NSString stringWithFormat:@"正在输入: %@", ret];
    }
...
- (NSAttributedString *)textWithWechatEmoji:(NSString *)text {
    ...
    ret = [JXEmotionEscape attributedEmojiStringWithText:text];
    if (!ret) {
...
- (NSString *)fileSizeDes {
    if (self.fileSize > 1024 * 1024) {
        if (self.type == JXMessageTypeFile) {
            return [NSString stringWithFormat:@"大小: %.2fM", (float)self.fileSize / (1024 * 1024)];
        } else {
            return [NSString stringWithFormat:@"%.2fM", (float)self.fileSize / (1024 * 1024)];
        }
    } else if (self.fileSize > 1024) {
        if (self.type == JXMessageTypeFile) {
            return [NSString stringWithFormat:@"大小: %.2fK", (float)self.fileSize / 1024];
        } else {
        ...
        if (self.type == JXMessageTypeFile) {
            return [NSString stringWithFormat:@"大小: %.2fB", (float)self.fileSize];
        } else {
...
- (NSMutableAttributedString *)attributedText {
    ...
    if (self.type == JXMessageTypeForeseeComposing) {    // 预知消息添加正在输入前缀
        textToDisplay = [NSString stringWithFormat:@"正在输入: %@", self.textToDisplay];
    }
...
- (BOOL)hasHTMLTag {
    if (self.type != JXMessageTypeText) return NO;
    NSNumber *ret;
