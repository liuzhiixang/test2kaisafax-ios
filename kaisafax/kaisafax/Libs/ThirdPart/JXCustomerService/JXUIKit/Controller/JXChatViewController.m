...
@interface JXChatViewController ()<JXShowViewDelegate> {
...
- (instancetype)initWithChatter:(NSString *)chatter andChatType:(JXChatType)chatType {
    if (self = [super init]) {
        ...
        _messageQueue = dispatch_get_main_queue();    // TODO: move to other thread
    }
    return self;
}
...
- (void)showMenuViewController:(JXCommonMessageCell *)showInView andMessage:(JXMessage *)message {
    ...
    if ([items count] > 0) {
        ...
        CGRect targetRect = CGRectMake(showInView.bubbleView.origin.x, showInView.origin.y,
                                       showInView.bubbleView.width, showInView.height);
...
- (NSArray *)editMenuItemsForMessage:(JXMessage *)message {
    ...
    // UIMenuItem *mutipleSelected =
    //        [[UIMenuItem alloc] initWithTitle:@"多选" action:@selector(mutipleSelect:)];
...
- (void)copyCell:(id)sender {
    ...
    [pasteboard setString:message.textToDisplay];
}
...
- (UIView *)messageToolbar {
    ...
    _messageToolbar = [[JXMessageToolbar alloc]
            initWithFrame:CGRectMake(0, self.view.frame.size.height - toolbarHeight,
    ...
    tableFrame.size.height = self.view.frame.size.height - _messageToolbar.frame.size.height -
...
- (void)addMessage:(JXMessage *)message {
    ...
    WEAKSELF;
    dispatch_async(_messageQueue, ^{
        NSArray *messages = [weakSelf organizeMessages:@[ message ]];
        ...
        for (id obj in messages) {
            ...
            [indexPaths addObject:[NSIndexPath indexPathForRow:weakSelf.dataSource.count - 1
                                                     inSection:0]];
        }
        NSInteger messageIndex = weakSelf.dataSource.count - 1;
        message.indexInTableView = messageIndex;
        [weakSelf.tableView insertRowsAtIndexPaths:indexPaths
                                  withRowAnimation:UITableViewRowAnimationNone];
        [weakSelf.tableView scrollToRowAtIndexPath:indexPaths.lastObject
...
- (void)messageFileDownloadFailed:(NSString *)messageID {
    ...
    JXMessage *message = [_conversation messageForId:messageID];
    message.progress = -1;
    [sJXHUD showMessage:@"下载失败" duration:0.8];
    ...
    if (resultArray.count) {
        if ([UIDevice currentDevice].systemVersion.floatValue < 8.0) {
            ...
            UIAlertController *alertController =
                    [UIAlertController alertControllerWithTitle:@"请选择"
                                                        message:@""
...
- (void)messageCell:(JXMessageCell *)cell fileMessageIconViewSelected:(JXMessage *)message {
    ...
    if ([data length] < message.fileSize) {
        ...
        if (date < message.expiredTime) {
            ...
            [sJXHUD showMessage:@"文件已过期" duration:0.8];
        }
    }
}
