...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = self.locationDescribe ? self.locationDescribe : @"位置";
    if (self.currentLocation) {
        [self setupDefaultLeftButtonItem];
    } else {
        WEAKSELF;
        [self setupDefaultLeftButtonItemWithTitle:@"取消"];
        [self setupRightBarButtonItemWithTitle:@"发送"
                                     andAction:^(id sender) {
    ...
    if (!self.currentLocation) {
        if ([CLLocationManager locationServicesEnabled]) {
            ...
            [sJXHUD showMessageWithActivityIndicatorView:@"定位中..." inView:self.view];
        } else {
            [sJXHUD showMessage:@"请在设置-隐私-定位服务中开启定位功能！" duration:1.7];
        }
    }
}
...
- (void)viewWillLayoutSubviews {
    ...
    _mapView.frame = CGRectMake(0, 0, self.view.width, self.view.height);
}

- (void)sendCurrentLocation:(id)sender {
    if (!self.currentLocation) {
        sJXHUDMes(@"没有获取到位置信息！", 1.4);
        return;
    }
