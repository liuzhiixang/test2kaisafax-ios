...
- (void)didFinishRecordingVoiceAction:(UIView *)recordView {
    ...
    [self.voiceRecorder stopWithCompletion:^(JXRecordErrorType type) {
        if (type == JXRecordErrorTypeRecordTimeToShort) {
            [[JXHUD sharedHUD] showMessage:@"录音时间过短，无法发送" duration:1.0];
...
- (void)didSelectedPhotoAction:(UIView *)moreView {
    if ([UIImagePickerController
        ...
        [sJXHUD showMessage:@"访问手机相册已被禁用，请到设置里打开" duration:1.6];
    }
}
...
- (void)didSelectedCameraAction:(UIView *)moreView {
    if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        [sJXHUD showMessage:@"相机已被禁用，请到设置里打开" duration:1.6];
    }
}
...
didFinishPickingMediaWithInfo:(NSDictionary<NSString *, id> *)info {
    ...
    if ([info[UIImagePickerControllerMediaType] isEqualToString:@"public.movie"]) {
        ...
        NSString *mp4FilePath = [[recordPath stringByDeletingPathExtension] stringByAppendingPathExtension:@"mp4"];
        ...
        if ([JXVideoConverter convertMovToMP4:info[UIImagePickerControllerMediaURL]
            ...
            if ([NSData dataWithContentsOfFile:mp4FilePath].length > 1024 * 1024 * 5) {
                ...
                sJXHUDMes(@"视频过大无法发送", 1.3);
                [picker dismissViewControllerAnimated:YES completion:nil];
                return;
            }
            // 获取缩略图
            WEAKSELF;
            [[JXVideoConverter sharedInstance] convertVideoFirstFrameWithURL:[NSURL fileURLWithPath:mp4FilePath] finishBlock:^(id image) {
                if ([UIImagePNGRepresentation([image fixOrientation]) writeToFile:thunbFilePath atomically:YES]) {
                    [weakSelf sendVideoMessageWithLocalPah:mp4FilePath thumbPath:thunbFilePath duration:duration];
                }
                [picker dismissViewControllerAnimated:YES completion:nil];
            }];
        }
