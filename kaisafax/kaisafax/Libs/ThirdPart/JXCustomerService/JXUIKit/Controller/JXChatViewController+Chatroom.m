...
- (void)setUserCount:(NSInteger)count {
    if (count > self.chatroom.maxMembers) {    // 聊天室人数已满owner进入人数不加1
...
- (void)joinChatroom {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.conversation.type == JXChatTypeRoom) {
            [self showMessageWithActivityIndicator:@"正在进入聊天室"];
...
- (void)didJoinChatroom:(NSString *)roomId withError:(JXError *)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        [self hideHUD];
        if (error) {
            sJXHUDMes([error getLocalDescription], 1.5f);
...
- (void)peopleJoinChatroom:(NSString *)useName {
    dispatch_async(dispatch_get_main_queue(), ^{
        [sJXHUD showMessage:[NSString stringWithFormat:@"%@加入了聊天室", useName] duration:1.6];
...
- (void)peopleLeaveChatroom:(NSString *)useName {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *textDisplay = [NSString stringWithFormat:@"%@离开了聊天室", useName];
...
- (void)chatroomDidDestory:(NSString *)roomId {
    dispatch_async(dispatch_get_main_queue(), ^{
        NSString *messasge = [NSString stringWithFormat:@"%@聊天室已解散", self.chatroom.subject];
