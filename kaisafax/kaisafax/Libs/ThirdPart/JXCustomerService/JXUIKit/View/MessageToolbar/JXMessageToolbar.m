...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _defaultEmoji = [JXEmoji allEmoji];
...
- (void)_setupSubviews {
    ...
    _backgroundImageView.image = [UIImage imageWithColor:[UIColor whiteColor]];
...
- (BOOL)textView:(UITextView *)textView
    ...
    if ([text isEqualToString:@"\n"]) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            NSString *attStr = [JXEmotionEscape mutableStringWithText:textView.text];
...
- (void)selectedFacialView:(NSString *)str isDelete:(BOOL)isDelete {
    ...
    if (!isDelete && str.length > 0) {
        ...
        [attr insertAttributedString:[JXEmotionEscape attStringFromTextForInputView:str]
    ...
    if (chatText.length > 0) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            if (![_inputTextView.text isEqualToString:@""]) {
                ...
                attStr = [JXEmotionEscape mutableStringWithText:attStr];
...
- (void)moreViewAddPhotoItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"图片"
...
- (void)moreViewAddCameraItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    JXToolBarMoreView *moreView = (JXToolBarMoreView *)self.moreView;
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"拍照"
...
- (void)moreViewAddVideoItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    JXToolBarMoreView *moreView = (JXToolBarMoreView *)self.moreView;
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"小视频"
...
- (void)moreViewAddLocationItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    JXToolBarMoreView *moreView = (JXToolBarMoreView *)self.moreView;
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"位置"
...
- (void)moreViewAddAudioCallItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    JXToolBarMoreView *moreView = (JXToolBarMoreView *)self.moreView;
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"语音通话"
...
- (void)moreViewAddVideoCallItemWithTitle:(NSString *)title andImage:(UIImage *)image {
    ...
    JXToolBarMoreView *moreView = (JXToolBarMoreView *)self.moreView;
    WEAKSELF;
    [moreView addItemWithTitle:title.length ? title : @"视频通话"
                      andImage:image ? image : JXChatImage(@"moreIcon_videaChat")
                     andAction:^(NSInteger index) {
