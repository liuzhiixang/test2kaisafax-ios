...
- (id)initWithTitle:(NSString *)title image:(UIImage *)image
{
    ...
    if (self) {
        ...
        [self setTitleColor:NUI_HELPER.appLightGrayColor forState:UIControlStateNormal];
...
- (void)layoutSubviews
{
    ...
    CGRect imageRect = CGRectMake(width * 0.3, width * 0.1, width * 0.4, width * 0.4);
    self.imageView.frame = imageRect;
    
    float labelHeight = height - (imageRect.origin.y + imageRect.size.height + width * 0.1);
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _cancelButton.layer.borderColor = NUI_HELPER.appBorderColor.CGColor;
        ...
        [_cancelButton setTitleColor:UIColorFromHex(0x4c4c4e) forState:UIControlStateNormal];
...
- (void)setStyle:(SGActionViewStyle)style{
    ...
    for (SGGridItem *item in self.items) {
        [item setTitleColor:NUI_HELPER.appLightGrayColor forState:UIControlStateNormal];
    }
}
...
- (void)layoutSubviews
{
    ...
    self.cancelButton.frame = CGRectMake(10/*self.bounds.size.width*0.05*/, self.titleLabel.bounds.size.height + self.contentScrollView.bounds.size.height - 10, self.bounds.size.width-20, 40);
...
- (void)layoutContentScrollView
{
    ...
    CGSize itemSize = CGSizeMake((self.bounds.size.width - margin.left - margin.right) / 4, 95);
